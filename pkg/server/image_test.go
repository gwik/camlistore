...
func TestImageThumbnailNoCaches(t *testing.T) {
	...
	if resp.Code != 200 {
		errContent, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			t.Fatal(err)
		}
		t.Fatalf("Expected 200 status code was %d: %s", resp.Code, errContent)
	}
	header := resp.Header()
...
func TestCheckIsAVideo(t *testing.T) {
	storage := new(test.Fetcher)
	b := &test.Blob{Contents: "This is text."}
	ref := b.BlobRef()
	storage.AddBlob(b)
	maxWidth := 40
	maxHeight := 40
	...
	handler.ServeHTTP(resp, req, ref)
	if resp.Code != 500 {
		t.Fatalf("Expected 500 status code was %d", resp.Code)
	}
}
